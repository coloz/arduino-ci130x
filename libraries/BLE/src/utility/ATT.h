...
#if BLE_USE_ESPLINK
// Upstream CCCD, SMP and prepared-write state is shared between peers.
// Keep one peer until these paths have independent per-peer storage.
#define ATT_MAX_PEERS 1
#elif DM_CONN_MAX
...
#if BLE_USE_ESPLINK
  // Drop local peers without issuing commands to a vanished controller.
