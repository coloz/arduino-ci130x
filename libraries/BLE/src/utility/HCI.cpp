...
void HCIClass::end()
{
  HCITransport.end();
#if BLE_USE_ESPLINK
  _recvIndex = _pendingPkt = _l2CapPduBufferSize = 0;
  ATT.resetConnections();
  GAP.resetState();
#endif
}
...
{
#if !BLE_USE_ESPLINK && defined(ARDUINO_AVR_UNO_WIFI_REV2)
...
#if BLE_USE_ESPLINK
    // Preserve partial H4 packets, but let a busy scanner yield to the sketch.
    if (++bytesRead > 2048 || uint32_t(millis() - pollStarted) >= 25) break;
#endif
    byte b = HCITransport.read();

    if (_recvIndex >= (int)sizeof(_recvBuffer)) {
#if BLE_USE_ESPLINK
        BLEESPLink.fail(BLE_ESPLINK_INVALID_PACKET);
        break;
#endif
        _recvIndex = 0;
        if (_debug) {
...
#if BLE_USE_ESPLINK
    if (_recvBuffer[0] == HCI_ACLDATA_PKT && _recvIndex == 5 &&
        (_recvBuffer[4] != 0 || _recvBuffer[3] > 251 || _recvBuffer[3] == 0)) {
      BLEESPLink.fail(BLE_ESPLINK_INVALID_PACKET);
      break;
    }
#endif
...
#ifndef __AVR__
#if BLE_USE_ESPLINK
    if (pktLen < 32 || !maxPkt) return -1;
    ATT.setMaxMtu(min(uint16_t(242), uint16_t(pktLen - 9)));
#else
...
{
  #if BLE_USE_ESPLINK
  if (!HCITransport.healthy() || !_maxPkt || plen > 242) return -1;
  const uint32_t creditStarted = millis();
  #endif
  while (_pendingPkt >= _maxPkt) {
    poll();
#if BLE_USE_ESPLINK
    if (!HCITransport.healthy()) return -1;
    if (uint32_t(millis() - creditStarted) >= 1000) {
      BLEESPLink.fail(BLE_ESPLINK_CREDIT_TIMEOUT);
      return -1;
    }
    delay(1);
#endif
  ...
  #if BLE_USE_ESPLINK
  if (HCITransport.write(txBuffer, sizeof(aclHdr) + plen) != sizeof(aclHdr) + plen) return -1;
  _pendingPkt++;
  #else
  _pendingPkt++;
...
int HCIClass::sendCommand(uint16_t opcode, uint8_t plen, void* parameters)
{
  ...
  #if BLE_USE_ESPLINK
  if (!HCITransport.healthy() ||
      HCITransport.write(txBuffer, sizeof(pktHdr) + plen) != sizeof(pktHdr) + plen) return -1;
  #else
...
#if BLE_USE_ESPLINK
    if (!HCITransport.healthy()) return -1;
    if (_cmdCompleteOpcode != opcode) delay(1);
#endif
  }
...
void HCIClass::handleAclDataPkt(uint8_t plen, uint8_t pdata[])
{
...
#if BLE_USE_ESPLINK
  if (plen < sizeof(HCIACLHdr) || aclHeader->dlen > size_t(plen) - sizeof(HCIACLHdr)) {
    BLEESPLink.fail(BLE_ESPLINK_INVALID_PACKET);
    return;
  }
#else
  ...
  if (pbFlag == 0b10) {
    ...
    if (_l2CapPduBufferSize == 0 ||
        aclHeader->dlen > sizeof(_l2CapPduBuffer) - _l2CapPduBufferSize) {
      BLEESPLink.fail(BLE_ESPLINK_INVALID_PACKET);
      return;
    }
#endif
...
#if BLE_USE_ESPLINK
  if (l2CapHeader->len > sizeof(_l2CapPduBuffer) - 4 ||
      l2CapPduSize > size_t(l2CapHeader->len) + 4) {
    BLEESPLink.fail(BLE_ESPLINK_INVALID_PACKET);
    return;
  }
#endif
...
#endif
#if BLE_USE_ESPLINK
    if(encryptionChange->status == 0 && encryptionChange->enabled > 0){
#else
    ...
    switch((LE_META_EVENT)leMetaHeader->subevent){
      ...
      case READ_LOCAL_P256_COMPLETE:{
        ...
        if(evtReadLocalP256Complete->status == 0x0){
          ...
          // Controller connection handles are not indexes into the peer array.
...
AuthReq HCIClass::localAuthreq(){
  ...
  local.setCT2(true);
#if BLE_USE_ESPLINK
  // Respect whether an application installed all persistent-key callbacks.
  return local;
#else
