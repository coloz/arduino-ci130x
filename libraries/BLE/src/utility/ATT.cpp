...
{
#if BLE_USE_ESPLINK
  if (!dlen || !data || !connected(connectionHandle)) return;
#endif
...
#if BLE_USE_ESPLINK
void ATTClass::resetConnections()
{
  static bool resetting = false;
  if (resetting) return;
  resetting = true;
  for (int i = 0; i < ATT_MAX_PEERS; ++i) {
    if (_peers[i].connectionHandle != 0xffff) removeConnection(_peers[i].connectionHandle, 0x16);
  }
  _pendingResp.connectionHandle = 0xffff;
  _pendingResp.length = 0;
  _longWriteHandle = _longWriteValueLength = 0;
  if (_longWriteValue) { free(_longWriteValue); _longWriteValue = NULL; }
  holdBufferSize = writeBufferSize = 0;
  _cnf = false;
  resetting = false;
}
#endif
...
{
#if BLE_USE_ESPLINK
  if (!handle || handle > GATT.attributeCount() || length < 0) return 0;
  BLELocalAttribute* attribute = GATT.attribute(handle - 1);
  if (!attribute || attribute->type() != BLETypeCharacteristic) return 0;
  BLELocalCharacteristic* characteristic = (BLELocalCharacteristic*)attribute;
  const bool requiresEncryption = characteristic->permissions() & (BLEPermission::BLEEncryption >> 8);
#endif
  ...
  for (int i = 0; i < ATT_MAX_PEERS; i++) {
...
#if BLE_USE_ESPLINK
    if (requiresEncryption && !(_peers[i].encryption & PEER_ENCRYPTION::ENCRYPTED_AES)) continue;
#endif
    ...
    #if BLE_USE_ESPLINK
    if (HCI.sendAclPkt(_peers[i].connectionHandle, ATT_CID, notificationLength, notification) != 0) continue;
    #else
...
int ATTClass::handleInd(uint16_t handle, const uint8_t* value, int length)
{
  ...
  for (int i = 0; i < ATT_MAX_PEERS; i++) {
    ...
    #if BLE_USE_ESPLINK
    if (HCI.sendAclPkt(_peers[i].connectionHandle, ATT_CID, indicationLength, indication) != 0) continue;
    const uint32_t indicationStarted = millis();
    #else
    ...
    while (!_cnf) {
#if BLE_USE_ESPLINK
      if (uint32_t(millis() - indicationStarted) >= _timeout) break;
      delay(1);
#endif
      HCI.poll();
...
void ATTClass::readOrReadBlobReq(uint16_t connectionHandle, uint16_t mtu, uint8_t opcode, uint8_t dlen, uint8_t data[])
{
...
#if BLE_USE_ESPLINK
        // ATT errors finish the request. The peer must retry after pairing;
        // never replay a global pending response into a later connection.
        return;
#else
        holdResponse = true;
#endif
  ...
  if(holdResponse){
#if BLE_USE_ESPLINK
    if (responseLength > sizeof(holdBuffer)) {
      sendError(connectionHandle, opcode, handle, ATT_ECODE_INSUFF_RESOURCES);
      return;
    }
#endif
...
void ATTClass::readByTypeReq(uint16_t connectionHandle, uint16_t mtu, uint8_t dlen, uint8_t data[])
{
  ...
  for (uint16_t i = (readByTypeReq->startHandle - 1); i < GATT.attributeCount() && i <= (readByTypeReq->endHandle - 1); i++) {
    ...
    if (attribute->type() == readByTypeReq->uuid) {
      ...
      BLELocalCharacteristic* characteristic = (BLELocalCharacteristic*)attribute;
#if BLE_USE_ESPLINK
      if (handle != characteristic->valueHandle()) continue;
      if (!(characteristic->properties() & BLERead)) {
        sendError(connectionHandle, ATT_OP_READ_BY_TYPE_REQ, handle, ATT_ECODE_READ_NOT_PERM);
        return;
      }
      if ((characteristic->permissions() & (BLEPermission::BLEEncryption >> 8)) &&
          !(getPeerEncryption(connectionHandle) & PEER_ENCRYPTION::ENCRYPTED_AES)) {
        sendError(connectionHandle, ATT_OP_READ_BY_TYPE_REQ, handle, ATT_ECODE_INSUFF_ENC);
        return;
      }
#endif
...
void ATTClass::writeReqOrCmd(uint16_t connectionHandle, uint16_t mtu, uint8_t op, uint8_t dlen, uint8_t data[])
{
  ...
  if (attribute->type() == BLETypeCharacteristic) {
    ...
    if((characteristic->permissions() &( BLEPermission::BLEEncryption >> 8)) > 0 &&
       (getPeerEncryption(connectionHandle) & PEER_ENCRYPTION::ENCRYPTED_AES) == 0){
#if BLE_USE_ESPLINK
      if (withResponse) sendError(connectionHandle, ATT_OP_WRITE_REQ, handle, ATT_ECODE_INSUFF_ENC);
      return; // write commands cannot elicit an ATT response or be replayed later
#else
      holdResponse = true;
...
#if BLE_USE_ESPLINK
    if (valueLength != 2 || handle < 2) {
      if (withResponse) sendError(connectionHandle, ATT_OP_WRITE_REQ, handle, ATT_ECODE_INVALID_PDU);
      return;
    }
#endif
...
void ATTClass::prepWriteReq(uint16_t connectionHandle, uint16_t mtu, uint8_t dlen, uint8_t data[])
{
  ...
  if ((characteristic->permissions() & (BLEPermission::BLEEncryption >> 8)) &&
      !(getPeerEncryption(connectionHandle) & PEER_ENCRYPTION::ENCRYPTED_AES)) {
    sendError(connectionHandle, ATT_OP_PREP_WRITE_REQ, handle, ATT_ECODE_INSUFF_ENC);
    return;
  }
#endif
...
#if BLE_USE_ESPLINK
    uint8_t* resized = valueSize > 0 ? (uint8_t*)realloc(_longWriteValue, valueSize) : NULL;
    if (!resized) {
      sendError(connectionHandle, ATT_OP_PREP_WRITE_REQ, handle, ATT_ECODE_INSUFF_RESOURCES);
      return;
    }
    _longWriteValue = resized;
#else
...
void ATTClass::execWriteReq(uint16_t connectionHandle, uint16_t mtu, uint8_t dlen, uint8_t data[])
{
  ...
  if (_longWriteHandle && (flag & 0x01)) {
    ...
    if ((characteristic->permissions() & (BLEPermission::BLEEncryption >> 8)) &&
        !(getPeerEncryption(connectionHandle) & PEER_ENCRYPTION::ENCRYPTED_AES)) {
      sendError(connectionHandle, ATT_OP_EXEC_WRITE_REQ, _longWriteHandle, ATT_ECODE_INSUFF_ENC);
      _longWriteHandle = _longWriteValueLength = 0;
      return;
    }
#endif
...
int ATTClass::sendReq(uint16_t connectionHandle, void* requestBuffer, int requestLength, uint8_t responseBuffer[])
{
  ...
  #if BLE_USE_ESPLINK
  if (HCI.sendAclPkt(connectionHandle, ATT_CID, requestLength, requestBuffer) != 0) {
    _pendingResp.connectionHandle = 0xffff;
    return 0;
  }
  #else
