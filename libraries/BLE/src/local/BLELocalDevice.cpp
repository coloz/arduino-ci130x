...
#if !BLE_USE_ESPLINK && (defined(ARDUINO_AVR_UNO_WIFI_REV2) || defined(ARDUINO_SAMD_MKRWIFI1010) || defined(ARDUINO_SAMD_NANO_33_IOT) || defined(TARGET_NANO_RP2040_CONNECT))
...
{
#if BLE_USE_ESPLINK
  // Disconnect callbacks run while their local GATT objects are still valid.
  HCI.end();
  GATT.end();
#else
  GATT.end();
...
#if !BLE_USE_ESPLINK && (defined(ARDUINO_PORTENTA_H7_M4) || defined(ARDUINO_PORTENTA_H7_M7) || defined(ARDUINO_NICLA_VISION) || defined(ARDUINO_GIGA) || defined(ARDUINO_OPTA))
  digitalWrite(BT_REG_ON, LOW);
#endif
