...
`true`。`end()` 会清除完成回调、异步提交停止请求并恢复静音前的音量，但不会关闭共享
SDK。
...
| `stop()` | 异步提交停止请求；未初始化时视为已停止 |
| `isPlaying()` | 查询当前是否正在播放或仍有排队请求 |
| `isReady()` | 查询 `begin()` 是否已成功完成 |

播放函数返回 `true` 只表示库的 FreeRTOS 播放队列已经复制请求，不代表随后在播放
任务中执行的资源查找和播放一定成功。参数无效、尚未初始化或队列已满时返回 `false`。
所有播放函数和 `stop()` 都不会等待底层播放器，因而不会阻塞 sketch 主程序。

`interruptCurrent` 默认为 `true`，请求会进入高优先级抢占队列；传入 `false` 会将
请求加入有序播放队列。专用任务会等待当前完整请求结束后，才取出下一条有序请求。
有序队列可等待 8 条请求，抢占队列可等待 4 条；相应队列已满时入队返回 `false`。
`optionIndex` 默认为 `-1`，表示使用资源包配置的默认选项。
...
播放任务会把一个数值的全部词元作为一个逻辑组合交给 SDK，并且整组数字完成后才取出
下一个数值；每组数值只回调一次。
...
SDK 原本会在持有提示音互斥锁时触发完成通知。Arduino 集成会先记录该事件，等待 SDK
解锁，再唤醒专用播放任务。因此用户回调始终在 `ChipIntelliAudio` 播放任务中执行，
每个逻辑请求回调一次，而不是每个数字词元回调一次。回调仍应保持简短，不要调用
`delay()`、等待锁、执行 Flash 写入或大量打印。建议只设置 `volatile` 标志或发送
非阻塞队列消息，再由 `loop()` 处理。通知不含播放成功/失败状态；被中断或后续资源
查找失败也可能产生通知。调用 `onFinished(nullptr)` 可清除回调。

## 示例

- `PlayVoiceId`：初始化播放器，按语音 ID 播放并安全处理完成事件；
- `VariableNumber`：主程序继续运行时异步排队播放一个整数和两个小数；
